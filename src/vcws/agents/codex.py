...
from .base import CancelNotSupported, TurnState
...
class CodexAgentTurn:
    """AgentTurn implementation wrapping CodexAgentBackend.process_turn logic."""
    ...
    def __init__(
        ...
        self.state: TurnState = TurnState.RUNNING
        self.kill_event: threading.Event = threading.Event()
        self._turn_id: str | None = None

    def __enter__(self) -> "CodexAgentTurn":
    ...
    def run(self) -> TurnOutcome:
        outcome = self._backend.process_turn(
            conversation=self._conversation,
            workspace_path=self._workspace_path,
            prompt=self._prompt,
            existing_thread_id=self._existing_thread_id,
    ...
    def cancel(self) -> None:
        if not self.supports_cancel:
            raise CancelNotSupported("CodexAgentTurn does not support cancel")
        self.state = TurnState.CANCELLED
        if self._turn_id is not None:
            try:
                self._backend._client.notify("turn/cancel", {"turnId": self._turn_id})
            except Exception:
                pass

    def deltas(self) -> Iterator[str]:
        raise NotImplementedError("deltas() not supported for CodexAgentTurn")
...
# Legacy alias
CodexAppServerBackend = CodexAgentBackend
