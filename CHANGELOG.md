...
- **CLI**: `serve` now **requires** `--agent {codex,claude-code,opencode}`. Running `python -m vcws serve` without `--agent` will exit non-zero with an argparse error.
...
- **Module**: `vcws.codex_app_server` is now a compatibility shim. Import from `vcws.agents.codex` or `vcws.agents` directly. The shim is scheduled for removal in 0.3 and is not guaranteed pickle-stable.
...
- `AgentBackend` + `AgentTurn` Protocols (`src/vcws/agents/base.py`) with `TurnState` enum (`RUNNING | COMPLETED | CANCELLED | KILLED`) and per-turn `kill_event`.
...
- PID lockfile at `.omx/runtime/serve.lock` prevents double-start. Live PID → always refuse. Stale PID → refuse unless `--force` or `VCWS_TAKEOVER_STALE=1`.
- Stop commands:
